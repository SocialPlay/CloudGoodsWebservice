...
namespace CloudGoods.CurrencyPurchase
{
    ...
    public AndroidJavaObject cls_StorePurchaser;
        ...
        if (message != "Fail")
        {
            ...
            bundlePurchaseRequest.UserID = AccountServices.ActiveUser.UserID;
            ...
            string bundleJsonString = JsonMapper.ToJson(bundlePurchaseRequest);//JsonConvert.SerializeObject(bundlePurchaseRequest);

            //CallHandler.InstancePurchaseCreditBundles(bundleJsonString, OnReceivedPurchaseResponse);
        }
        else
        {
            PurchasePremiumCurrencyBundleResponse response = new PurchasePremiumCurrencyBundleResponse();
